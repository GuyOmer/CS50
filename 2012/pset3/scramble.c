...
bool load(string s);
bool lookup(string s, int minpt, int maxpt);
void scramble(void);
...
int main(int argc, string argv[])
{
    ...
    {
        // clear the screen
        //clear();         //REMOVE COMMENT AFTER DEBUG
        ...
        {
            if (find(s) && lookup(s, 0, dictionary.size - 1))
                score += strlen(s);
        }
    }
...
bool lookup(string s, int minpt, int maxpt)
{
    ...
    int midpt = (minpt+maxpt)/2;      //finds mid point
    
    if (strcmp(s, dictionary.words[midpt].letters) ==-1)  //target is at lower part of array
            return lookup(s, minpt, midpt-1);
            
    else if (strcmp(s, dictionary.words[midpt].letters) ==1)  //target is at upper part of array
        return lookup (s, midpt+1, maxpt);
    
       //key found
    
        if(dictionary.words[midpt].found  == true)
            return false;
        else
        {
            dictionary.words[midpt].found  = true;
            return true;
        }
...
void scramble(void)
{
   int wgrid[DIMENSION][DIMENSION];   //copies grid into a work grid
   for (int i=0; i<DIMENSION; i++)  //rows
    {
        for (int j = 0; j<DIMENSION; j++) //cols
        {
   ...
   for (int i = 0; i < DIMENSION; i++)
   {
        for (int j = 0; j < DIMENSION; j++)
        {
            grid[i][DIMENSION-1-j] = wgrid[j][i];
        }
    }
}
